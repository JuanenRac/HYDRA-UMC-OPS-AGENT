...
> **ステータス: v0.0.9、scaffolding - 6 件の納品のうち第 1-5 弾(証拠、
...
**正直な現状確認 - 今日実際に動くもの:** すでに出荷済みの 5 つのデリバリー - エビデンス(`inventory.py`、`edge_agent.py`、`incident.py`)、診断(`diagnosis.py`)、人間が承認する変更(`change_proposal.py`)、カナリアデプロイ(`canary_deploy.py`)、そして検証(`verification.py`)、さらにマスキング境界(`log_redaction.py`)と CLI(`cli.py`) - はすべて本物であり、エンドツーエンドでテストされている(11 個のテストファイル全体で 136 件のテストと 6 件のサブテストが成功)。カナリアデプロイの段階は、モックではなく、実際に使い捨ての本物のローカル git リポジトリに対して検証されている。診断機能は模擬の `AIProvider` に対してしかテストされていない - `AnthropicProvider`/`OpenAIProvider` は同じプロトコルの本物の実装であるが、このテストスイートの中では、本物の Anthropic API や OpenAI API への実際のライブ呼び出しに対して検証されたことは一度もない(この環境には利用可能なキーがない)。エッジ役割とコントロールプレーン役割は、今日の時点では保存されたスナップショットファイルを介してのみやり取りしており、両者の間にはまだネットワーク伝送が存在しない。デリバリー 6(音声/通知)は単に先送りされているのではなく、実際に完全にブロックされている: HYDRA-UMC-VOICE-UI には今日、統合できるような本物のアウトバウンド通知インターフェースが存在しない。これまでに何が実際に出荷されたかは `CHANGELOG.md` を参照。
